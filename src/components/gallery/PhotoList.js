  ...
  return (
    <div className="py-10 px-5">
      <div className="grid grid-cols-4 gap-5">
        {photos.length > 0 &&
          photos.map((item, index) => <PhotoItem key={item.id} info={item}></PhotoItem>)}
      </div>
    </div>
  );
};
...
const PhotoItem = ({ info: { url, isFavorite, id } }) => {
  ...
  return (
    <div className="relative h-[300px] cursor-pointer group ">
      ...
      <span
        onClick={() => toggleFavorite(id)}
        className="w-8 absolute right-5 top-5 z-10 cursor-pointer opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all"
      >
        <svg
          width="42"
          height="38"
      ...
      <button
        onClick={() => addToCart(item)}
        className="py-3 text-sm font-medium px-6 bg-white rounded-lg text-black absolute z-10 bottom-5 left-2/4 -translate-x-2/4 
