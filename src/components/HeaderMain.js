...
//   const favoriteCount =
//     photos.length > 0 ? photos.filter((item) => item.isFavorite === true).length : 0;
...
import React from 'react';
import { useAuth } from '../contexts/auth-context';
import { useGallery } from '../contexts/gallery-context copy';

const HeaderMain = () => {
  const { user, setUser } = useAuth();
  const { photos, cardItems } = useGallery();

  const favoriteCount = photos.filter((item) => item.isFavorite === true).length;
  const cardItemCount = cardItems.length;
  return (
    <div className="p-4 bg-white shadow-md flex items-center justify-center">
      {user ? (
        <div className="flex items-center gap-x-3">
          <img src={user.avatar} alt="" className="w-10 h-10 rounded-full object-cover" />
          <span className="tetx-sm font-medium">
            Welcome back! <strong>{user.fullname}</strong>
          </span>
        </div>
      ) : (
      ...
      <div className="ml-auto mr-5 flex items-center gap-5">
        <span className="relative w-8">
          ...
          <span className="absolute right-0 top-0 translate-x-2/4 w-5 h-5 text-white text-xs bg-red-400 rounded-full flex items-center justify-center ">
            {cardItemCount || 0}
          </span>
        </span>
        <span className="relative w-8">
